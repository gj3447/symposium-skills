...
name: pi-workbench
description: >-
  Coordinate PI research and engineering through a canonical-main single-writer gate and direct, reproducible measurements. Use when: work touches `PI/`, its symlinked repositories, EngineBoy measurement, or a claim of PI implementation or research progress. Do not use when: research is elsewhere in SYMPOSIUM and has no PI measurement surface; use `$symposium-research` instead.
---

# PI Workbench

One root/parent session writes canonical `main`; parallel children remain
read-only. Keep measured observations separate from interpretations and claims.
...
## 3. Measure behavior directly

For a behavior change, lock the expected observable before the satisfying run,
execute the real code path, and record the command, exit status, output hash, and
relevant artifact hashes. Add a negative regression when it materially tests the
same behavior. A mock that bypasses the changed path or a hand-written success
...
Do not require a proprietary receipt format or a static scoring service.

## 4. Keep claims separate from measurements

For a scientific or programme-progress claim, freeze the raw evidence and state
the hypothesis, prediction, and failure condition separately. Local automation
may verify structure and hashes but must not manufacture a progress verdict.
Human or external review can consume the frozen evidence when explicitly asked;
it is not a mandatory completion layer.
...
readback, correlation ID, exact test commands and outcomes, artifact hashes,
provenance, and unresolved risks. `--verify-linked` checks packet-linked file
hashes but does not execute the claimed commands.
